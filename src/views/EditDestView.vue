...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`/api/destinations/${destId}`, updatedDest)
    // const response = await axios.put(`https://basic-backend.fly.dev/destinations/${destId}`, updatedDest)
...
onMounted(async () => {
  try {
    ...
    // populate inputs
    form.type = state.dest.type;
    form.title = state.dest.title;
    form.description = state.dest.description;
    form.best_time = state.dest.best_time;
    form.location = state.dest.location;
    form.company.name = state.dest.company.name;
    form.company.description = state.dest.company.description;
    form.company.contactEmail = state.dest.company.contactEmail;
    form.company.contactPhone = state.dest.company.contactPhone;
  } catch (error) {
    ...
    <div class="container m-auto max-w-2xl py-24">
      <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form @submit.prevent="handleSubmit">
          <h2 class="text-3xl text-center font-semibold mb-6">Edit Destination</h2>
